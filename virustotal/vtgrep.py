...
import logging
import webbrowser
from binaryninja import show_message_box, MessageBoxButtonSet, MessageBoxIcon
...
class VTGrepSearch(object):
    ...
    def __init__(self, *args, **kwargs):
        # Accept an optional BinaryView for Binary Ninja integration
        self.bv = kwargs.get("bv", None)
        self.string_searching = kwargs.get("string", False)
        if self.string_searching:
            if isinstance(self.string_searching, str):
        ...
        logging.debug("[VTGREP] VTGrepSearch initialized.")
        logging.debug(
            "[VTGREP] addr_start: %s, addr_end: %s",
            hex(self.addr_start),
            hex(self.addr_end),
        )
    ...
    def _create_query(self, wildcards, strict):
        ...
        if not Disassembler.valid_address_range(self.addr_start, self.addr_end):
            ...
            self.addr_start, self.addr_end, self._MAX_QUERY_SIZE
        ):
            ...
            if wildcards:  # Search for similar code
                if self.bv is None:
                    logging.error("[VTGREP] BinaryView required for wildcard searches.")
                    return None

                while current is not None and current < self.addr_end:
                    new_opcodes = Disassembler.get_opcodes(self.bv, current)
                    if new_opcodes == 0 or not new_opcodes:
                        break

                    str_buf += new_opcodes.strip() + " "

                    nxt = Disassembler.next_address(self.bv, current)
                ...
                # Raw bytes search
                if self.bv is None:
                    logging.error("[VTGREP] BinaryView required for byte searches.")
                    return None
                try:
                    raw = Disassembler.get_bytes(
                        self.bv, self.addr_start, self.addr_end
                    )
                except Exception as e:
                    logging.error("[VTGREP] get_bytes error: %s", e)
                    raw = None
                if raw:
                    str_buf = binascii.hexlify(raw).decode("utf-8")
    ...
    def search(self, wildcards=False, strict=False):
        ...
        logging.debug(
            "[VTGREP] Starting search, wildcards=%s, strict=%s", wildcards, strict
        )
        ...
        if str_buf is None:
            ...
            try:
                show_message_box(
                    "VT Grep",
                    "Invalid query length or area selected.",
            ...
            if len_query and self._MIN_QUERY_SIZE >= len_query:
                ...
                try:
                    show_message_box(
                        "VT Grep",
                        "The query produced is too short.",
                        MessageBoxButtonSet.OKButtonSet,
                        MessageBoxIcon.WarningIcon,
                    )
                except Exception:
                ...
                try:
                    show_message_box(
                        "VT Grep",
                        "The query produced is too long.",
                        MessageBoxButtonSet.OKButtonSet,
                        MessageBoxIcon.WarningIcon,
                    )
                except Exception:
                ...
                str_buf = "{" + str_buf + "}"
                logging.debug("[VTGREP] Final query string: %s", str_buf)
                vtgrep_url = "www.virustotal.com/gui/search/content:{}/files"
                url = "https://{}".format(quote(vtgrep_url.format(str_buf)))
                # mark the request coming from this Binary Ninja plugin
                url += "?utm=vt_bn_plugin"
